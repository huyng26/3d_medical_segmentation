...
def build_msd_dataloader(args,mode: str = "train", task: int = 2):
    ...
    if task == 2:
        data_path = os.path.join(data_path, "Task02_Heart")
    else:
        data_path = os.path.join(data_path, "Task09_Spleen")
    ...
    if mode == "train":
        train_transforms = build_msd_train_transforms(roi_size=tuple(args.img_size), crop_samples=args.crop_samples)
